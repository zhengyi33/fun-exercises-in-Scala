...
object fp3 {
  ...
  def removeDupes1 [X] (xs:List[X]) : List[X] = {
    ...
    xs match {
       case Nil => Nil
       case y::(z::zs) if (y == z) => removeDupes1(y::zs)
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    // TODO: Provide definition here.
    def aux(xs:List[X], count:Int) : List[(Int, X)] = {
    	xs match {
       	   case Nil => Nil
       	   case y::(z::zs) if (y == z) => aux(y::zs, count+1)
       	   case y::ys => (count, y)::aux(ys, 1)
    	}
    }
  ...
  def splitAt [X] (n:Int, xs:List[X]) : (List[X], List[X]) = {
    // TODO: Provide definition here.
    def aux(accum:List[X], xs:List[X], i:Int) : (List[X], List[X]) = {
    	xs match {
       	   case Nil => (accum, Nil)
       	   case y::ys if (i<n) => aux(accum:::List(y), ys, i+1)
	   case y::ys => (accum, xs)
	}
    }
    aux(Nil, xs, 0)
