...
object subtyping {
  ...
  def observeCounter (f : Counter => Unit) : Int = {
    ...
    class InnerCounter extends Counter {
    	  private var innerN = 0;
	  def getInnerN() : Int  = innerN;
	  override def increment () = { super.increment (); innerN = innerN + 1; }
	  override def decrement () = { super.decrement (); innerN = innerN + 1; }
    }
    val ic = new InnerCounter();
    f(ic);
    ic.getInnerN();
  }
  ...
  def observeCounterList (f : List[Counter] => Unit) : List[Int] = {
    ...
    val ics = List(new InnerCounter(), new InnerCounter(), new InnerCounter());
    f(ics);
    ics.map((ic:InnerCounter)=>ic.getInnerN());
  }
  ...
  def observeCounterArray (f : Array[Counter] => Unit) : Array[Int] = {
    ...
    val c1:Counter = new InnerCounter();
    val c2:Counter = new InnerCounter();
    val c3:Counter = new InnerCounter();
    val cs = Array(c1,c2,c3);
    f(cs);
    val ic1:InnerCounter = c1.asInstanceOf[InnerCounter];
    val	ic2:InnerCounter = c2.asInstanceOf[InnerCounter];
    val	ic3:InnerCounter = c3.asInstanceOf[InnerCounter];
    Array(ic1.getInnerN(),ic2.getInnerN(),ic3.getInnerN());
  }
}
